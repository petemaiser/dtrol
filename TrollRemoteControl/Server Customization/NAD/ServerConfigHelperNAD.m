...
//            }
        ...
        (self.serverSetupController.serverSetupStatus == ServerSetupStatusInnterogationSuccess))
    {
        ...
        if (self.zone4) {
            ...
            // Process any Tuner Overrides
            NSString *tunerOverrideZoneIP = [demoServer objectForKey:@"zone4.tunerOverrideZone.IP"];
            if (tunerOverrideZoneIP && ![tunerOverrideZoneIP isEqualToString:@""]) {
                NSString *tunerOverrideZoneNameLong = [demoServer objectForKey:@"zone4.tunerOverrideZone.nameLong"];
                RemoteZone *tunerOverrideZone =  [self getZoneWithServerIP:tunerOverrideZoneIP andZoneNameLong:tunerOverrideZoneNameLong];
                self.zone4.tunerOverrideZoneUUID = tunerOverrideZone.zoneUUID;
            ...
            // Process any Dependent Zones
            NSString *dependentZoneOverrideIP = [demoServer objectForKey:@"zone4.dependentZone.IP"];
            if (dependentZoneOverrideIP && ![dependentZoneOverrideIP isEqualToString:@""]) {
                NSString *dependentZoneNameLong = [demoServer objectForKey:@"zone4.dependentZone.nameLong"];
                RemoteZone *dependentZone =  [self getZoneWithServerIP:dependentZoneOverrideIP andZoneNameLong:dependentZoneNameLong];
                [self.zone4.dependentZoneUUIDList addObject:dependentZone.zoneUUID];
            }
        }
